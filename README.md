...
- **WireGuard** — two providers: ProtonVPN (a link and step-by-step guide in
  the card walks you through generating a config at protonvpn.com and pasting
  it in), and a generic WireGuard configurator that takes any standard client
  config -- e.g. one from [home-base](https://github.com/shrout1/home-base)'s
  "WireGuard Peers" card for your own server (shown once, never stored
  server-side), or any other WireGuard server/provider.
...
Connecting is always a deliberate, one-click action from the dashboard --
the VPN never auto-starts at boot, even if it was connected when the Pi was
last powered off. A tunnel enabled at boot would try to handshake before
there's any way to know the AP's clock is trustworthy yet or a captive
portal has been cleared, and WireGuard's replay protection rejects a
handshake timestamped under a skewed clock; combined with a "route
everything through the VPN" traffic mode, that can make the dashboard
itself look unreachable to AP clients, which is exactly the situation you'd
want the dashboard *for*. Every boot comes up with no tunnel and the
dashboard always reachable on the plain uplink; reconnect once you're
satisfied the clock and captive portal are sorted.
